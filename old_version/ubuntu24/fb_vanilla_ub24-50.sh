...
FB_VER=5.0
FB_URL="https://github.com/FirebirdSQL/firebird/releases/download/v5.0.3/Firebird-5.0.3.1683-0-linux-x64.tar.gz"
DBG_URL="https://github.com/FirebirdSQL/firebird/releases/download/v5.0.3/Firebird-5.0.3.1683-0-linux-x64-debugSymbols.tar.gz"
FTP_URL="https://cc.ib-aid.com/download/distr"
CRYPT_ARC=CryptPlugin-FB_50_LINUX_64bit.tar.xz
...
prepare_os(){
	...
	apt install --no-install-recommends -y ca-certificates net-tools wget unzip gettext libncurses6 curl tar tzdata locales sudo xz-utils file libtommath1 libicu74 bsdmainutils libssl-dev
...
extract_fb(){
	...
	tar xvf $TMP_DIR/Firebird-5*.gz -C $TMP_DIR/fb --strip-components=1 > /dev/null || exit_script 1 "Error unpacking FB archive"
...
if [ $CRYPT_ONLY -eq 1 ]; then
	...
	if [ $DEBUG -eq 1 ]; then
		...
		mkdir $TMP_DIR/debug
		tar xvf $TMP_DIR/Firebird-5.*-linux-x64-debugSymbols.tar.gz -C $TMP_DIR/debug > /dev/null || exit_script 1 "Error unpacking FB debug info"
