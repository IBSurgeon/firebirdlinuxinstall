...
FB_VER=4.0
FB_URL="https://github.com/FirebirdSQL/firebird/releases/download/v4.0.6/Firebird-4.0.6.3221-0.amd64.tar.gz"
DBG_URL="https://github.com/FirebirdSQL/firebird/releases/download/v4.0.6/Firebird-debuginfo-4.0.6.3221-0.amd64.tar.gz"
FTP_URL="https://cc.ib-aid.com/download/distr"
CRYPT_ARC=CryptPlugin-FB_40_LINUX_64bit.tar.xz
...
extract_fb(){
	...
	tar xvf $TMP_DIR/Firebird-4*.gz -C $TMP_DIR/fb --strip-components=1 > /dev/null || exit_script 1 "Error unpacking FB archive"
...
if [ $CRYPT_ONLY -eq 1 ]; then
	...
	if [ $DEBUG -eq 1 ]; then
		...
		mkdir $TMP_DIR/debug
		tar xvf $TMP_DIR/Firebird-debuginfo-4.*.amd64.tar.gz -C $TMP_DIR/debug > /dev/null || exit_script 1 "Error unpacking FB debug info"
